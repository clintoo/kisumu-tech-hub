...
import { Pencil, Trash2, Plus, LogOut, Calendar, Users } from "lucide-react";
import { fetchEvents, type Event } from "@/lib/events-api";

export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({ meta: [{ title: "Admin — Zone01 Kisumu Events" }, { name: "robots", content: "noindex" }] }),
  component: AdminPage,
});
...
function AdminPage() {
  ...
  async function toggleStatus(ev: Event) {
    const next = ev.status === "upcoming" ? "live" : ev.status === "live" ? "completed" : "upcoming";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-5">
        <Card className="p-8 text-center max-w-md">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Your account does not have admin permissions.</p>
          <Button className="mt-6" variant="hero" onClick={handleSignOut}>Sign out</Button>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/40 backdrop-blur sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-5 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="w-8 h-8 rounded-lg bg-gradient-blue flex items-center justify-center font-bold text-primary-foreground">Z</Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={handleSignOut}><LogOut className="w-4 h-4" /> Sign out</Button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-5 sm:px-8 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Create, edit and manage event listings.</p>
          </div>
        ...
        <div className="grid gap-3">
          ...
          {events.map((e) => (
            <Card key={e.id} className="p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-primary/40 transition-colors">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-display font-semibold">{e.title}</h3>
                  <Badge variant={e.status === "live" ? "default" : "outline"} className={e.status === "live" ? "bg-primary text-primary-foreground" : ""}>
                    {e.status}
                  </Badge>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" variant="ghost" onClick={() => { setEditing(e); setOpen(true); }}><Pencil className="w-4 h-4" /></Button>
      ...
      <EventDialog open={open} onOpenChange={setOpen} event={editing} onSaved={() => qc.invalidateQueries({ queryKey: ["events"] })} />
    </div>
  );
}

function EventDialog({ open, onOpenChange, event, onSaved }: { open: boolean; onOpenChange: (v: boolean) => void; event: Event | null; onSaved: () => void }) {
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input id="date_time" name="date_time" type="datetime-local" defaultValue={defaultDt} required />
            </div>
            <div>
          ...
          <div>
            ...
            <Input id="image_url" name="image_url" defaultValue={event?.image_url ?? ""} placeholder="https://..." />
          </div>
          <div>
            <Label htmlFor="registration_url">External registration URL (optional)</Label>
            <Input id="registration_url" name="registration_url" defaultValue={event?.registration_url ?? ""} placeholder="https://..." />
          </div>
...
function RegistrationsPanel() {
  ...
  return (
    <section className="mt-16">
      <h2 className="font-display text-2xl font-bold flex items-center gap-2"><Users className="w-5 h-5" /> Recent registrations</h2>
          ...
          <tbody>
            ...
            {regs.map((r: any) => (
              <tr key={r.id} className="border-t border-border">
                ...
                <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{r.events?.title ?? "—"}</td>
                <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{new Date(r.created_at).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
